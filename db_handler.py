# db_handler.py - Enhanced with Attendance Support
...
def show_attendance_data(date_info: Optional[Dict] = None, employee_name: Optional[str] = None, limit: int = 100) -> str:
    """Show attendance data from attendance.raw table"""
    handler = get_db_handler()
    conn = handler.connect('attendance')
    
    if not conn:
        return "❌ Could not connect to attendance database"
    ...
    try:
        ...
        # Add date filter (without using generate_date_sql for simpler handling)
        if date_info:
        ...
        if employee_name:
            where_clauses.append("`employee_name` LIKE %s")
        ...
        # Get total count
        count_query = "SELECT COUNT(*) as count FROM `raw`"
        if where_clauses:
            count_query += " WHERE " + " AND ".join(where_clauses)
        
        cursor.execute(count_query, tuple(params[:-1]) if params[:-1] else None)
        total_count = cursor.fetchone()['count']
        ...
        if date_info:
            output += f"**Period:** {date_info['description']}\n"
        
        output += f"**Total Records:** {total_count:,}\n"
        output += f"**Showing:** {min(len(results), limit)} records\n\n"
        
        if not results:
            output += "❌ **No attendance records found.**\n\n"
            if employee_name and date_info:
                output += f"**{employee_name}** was **NOT PRESENT** {date_info['description']}.\n\n"
            output += "💡 Try:\n"
            output += "- \"show attendance today\"\n"
            output += "- \"show attendance yesterday\"\n"
            output += "- \"is KE0008 present today\"\n"
            output += "- \"show John's attendance from last 7 days\"\n"
            output += "- \"attendance for Maria today\"\n"
        else:
            # Special message for presence check
            if employee_name and date_info and date_info['type'] == 'today':
                output += f"✅ **Yes, {employee_name} is PRESENT today!**\n\n"
            elif employee_name and date_info:
                output += f"✅ **Yes, {employee_name} was present {date_info['description']}!**\n\n"
            
            # Group by employee if showing multiple employees
            if not employee_name and len(results) > 0:
                from collections import defaultdict
                by_employee = defaultdict(list)
                for record in results:
                    by_employee[record.get('employee_name', 'Unknown')].append(record)
                
                output += "## 📊 Attendance by Employee:\n\n"
                for emp_name, records in sorted(by_employee.items()):
                    output += f"### {emp_name} ({len(records)} records)\n"
            ...
            output += "---\n\n"
            output += "## 💡 More Options:\n\n"
            output += "- \"show attendance from yesterday\"\n"
            output += "- \"show attendance from last 7 days\"\n"
            output += "- \"is KE0152 present today\"\n"
            output += "- \"show John's attendance today\"\n"
            output += "- \"attendance for Maria from 2024-12-01 to 2024-12-15\"\n"
